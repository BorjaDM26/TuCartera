<template>
  <b-navbar class="app-header" type="is-black">
    <template #brand>
      <b-navbar-item
        tag="router-link"
        :to="{ path: '/' }"
        class="app-header_logo-container"
      >
        <img
          class="app-header_logo"
          alt="Tu Cartera logo"
          src="../../assets/logo.svg"
        />
      </b-navbar-item>
    </template>
    <template #start v-if="userIsLogged">
      <b-navbar-item href="#">Carteras</b-navbar-item>
      <b-navbar-item href="#">Transacciones</b-navbar-item>
    </template>

    <template #end v-if="userIsLogged">
      <b-navbar-item tag="a" @click="logout">Cerrar sesión</b-navbar-item>
    </template>

    <template #end v-else>
      <b-navbar-item tag="a" @click="login">Registrarse</b-navbar-item>
      <b-navbar-item tag="a" @click="login">Iniciar sesión</b-navbar-item>
    </template>
  </b-navbar>
</template>

<script lang="ts">
import { Component, Vue } from 'vue-property-decorator';

@Component({
  name: 'AppHeader',
})
export default class AppHeader extends Vue {
  userIsLogged = true;

  private logout() {
    this.userIsLogged = false;
  }

  private login() {
    this.userIsLogged = true;
  }
}
</script>

<style scoped lang="scss">
.app-header {
  padding: 0 $navbar-h-pad;
  height: $header-height;

  &_logo {
    width: auto;
    min-height: $header-height;
    max-height: $header-height;
  }

  &_logo-container {
    padding: 0 $logo-h-pad;
  }
}
</style>
